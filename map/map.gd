...
func _ready() -> void:
	...
	plane_mesh.size = Vector2(self.get_aabb().size.x * self.scale.x, self.get_aabb().size.z * self.scale.z)
...
func set_map(raw_map):
	...
	var start_point = -self.get_aabb().size / 2 + Vector3(tile_size / 2, 0, tile_size / 2)
